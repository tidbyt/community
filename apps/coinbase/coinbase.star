"""
Applet: Coinbase
Summary: Coinbase Balance Tracker
Description: Displays your current Coinbase holdings and balances.
Author: harrywynn
"""

load("cache.star", "cache")
load("encoding/base64.star", "base64")
load("encoding/json.star", "json")
load("http.star", "http")
load("humanize.star", "humanize")
load("render.star", "render")
load("schema.star", "schema")
load("secret.star", "secret")

COINBASE_LOGO = base64.decode("")
COINBASE_CLIENT_SECRET = secret.decrypt("AV6+xWcEJMC6gX7GH0hewzGQ/B/2l0jpAtMyD1BZxfZLcL2vF9ZPHsaL+/TK9l7tDVs700mxxmJERtbBtGwLWE2RQPY8KdF88yWsYABohFu7GgfqjQ5LX0pqv6wrTqWfESyA7fPk7c3fgqOz3Hrr6AUrkddIDJuWpeZ8wywhZTYJad7q/Pd6RqsNVgX9qvhzOoFODy9O83rMM8GlKCZS6VHcIZG5DQ==")
COINBASE_CLIENT_ID = secret.decrypt("AV6+xWcE2JaB2O2ordBy0T1hLm/fb3TSEfrEpGojMFF9kazLU04l+4czjZcjg10VMPjMyr884buJOXY070nmqgiP55bdN7ZtT484avoKQqoF8aM3VnE4Y1Jw//51a36IEk2/uJBmqGfY90H7QlYBQ76zVAL5DWkuVLGv6fI7Ux12Y7tv02gmWN77L+Hq+/GrfuV5I9c0wwM4nj5t/Zu1BIwrn3WPAQ==")

ACCOUNTS_CACHE_KEY = "accounts.cache"
PRICE_CACHE_KEY = "price.cache"
AUTH_TOKEN = None

def main(config):
    if config.get("token") == None:
        return render.Root(
            child = render.Text("Please login"),
        )

    # load exchange rates
    rates = cache.get(PRICE_CACHE_KEY)

    if rates != None:
        # cached rates
        rates = json.decode(rates)
    else:
        # get current exchange rates
        res = http.get("https://api.coinbase.com/v2/exchange-rates")

        if res.status_code != 200:
            return render.Root(
                child = render.Text("Rates unavailable!"),
            )
        else:
            # cache for 15 minutes
            rates = res.json()["data"]["rates"]
            cache.set(PRICE_CACHE_KEY, json.encode(rates), ttl_seconds = 900)

    accounts = cache.get(ACCOUNTS_CACHE_KEY)

    if accounts != None:
        # cached accounts
        accounts = json.decode(accounts)
    else:
        AUTH_TOKEN = config.get("token")

        # load account balances
        res = http.get("https://coinbase.com/api/v3/brokerage/accounts?limit=250", headers = {
            "Authorization": "Bearer " + AUTH_TOKEN,
        })

        if res.status_code != 200:
            return render.Root(
                child = render.Text("Accounts unavailable!"),
            )
        else:
            # cache for 15 minutes
            accounts = res.json()["accounts"]
            cache.set(ACCOUNTS_CACHE_KEY, json.encode(accounts), ttl_seconds = 900)

    if accounts == None:
        return render.Root(
            child = render.Text("Accounts unavailable!"),
        )

    # for display
    currencies = []
    balance = 0.0

    # match account balances to rates
    for x in accounts:
        available = float(x["available_balance"]["value"])

        # only count if we have a balance
        if available > 0.0:
            balance += float(x["available_balance"]["value"]) // float(rates[x["currency"]])
            currencies.append(x["currency"])

    return render.Root(
        child = render.Column(
            main_align = "center",
            cross_align = "center",
            children = [
                render.Row(
                    main_align = "space_evenly",
                    cross_align = "center",
                    expanded = True,
                    children = [
                        render.Padding(
                            pad = (0, 2, 0, 3),
                            child = render.Text(
                                content = ("$" + humanize.ftoa(num = balance, digits = 2)),
                                font = "6x13",
                            ),
                        ),
                    ],
                ),
                render.Row(
                    main_align = "space_evenly",
                    cross_align = "center",
                    expanded = True,
                    children = [
                        render.Image(src = COINBASE_LOGO, width = 9),
                        render.Padding(
                            pad = (0, 2, 0, 0),
                            child = render.Marquee(
                                width = 52,
                                align = "center",
                                child = render.Text(
                                    content = " | ".join(currencies),
                                    font = "tom-thumb",
                                ),
                            ),
                        ),
                    ],
                ),
            ],
        ),
    )

def oauth_handler(params):
    params = json.decode(params)

    params["client_secret"] = COINBASE_CLIENT_SECRET or "fake-client-secret"

    res = http.post("https://api.coinbase.com/oauth/token", params = params)

    if res.status_code != 200:
        fail("token request failed with status code: %d - %s" % (res.status_code, res.body()))

    return res.json()["access_token"]

def get_schema():
    return schema.Schema(
        version = "1",
        fields = [
            schema.OAuth2(
                id = "token",
                name = "Coinbase Account",
                desc = "",
                icon = "",
                handler = oauth_handler,
                client_id = COINBASE_CLIENT_ID or "fake-client-id",
                authorization_endpoint = "https://www.coinbase.com/oauth/authorize",
                scopes = [
                    "wallet:accounts:read",
                ],
            ),
        ],
    )
