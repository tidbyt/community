...
# defines schema for configuration, which there is none for this application
def get_schema():
    return schema.Schema(
        version = OPM_STATUS_VERSION,
        fields = [
     ],
    )    
...
def main(config):
    ...
    if data == None:
        ...
        cache.set(OPM_JSON_URL + ".dataischached", 
            "YES",
            ttl_seconds = EXPIRE_URL_DATA)
        
        # store the opm status and applies to date
        cache.set(OPM_JSON_URL + ".status", 
            statussummary_val,
            ttl_seconds = EXPIRE_URL_DATA)
        cache.set(OPM_JSON_URL + ".appliesto",
            appliesto_val,
            ttl_seconds = EXPIRE_URL_DATA)
        cache.set(OPM_JSON_URL + ".type",
            statustype_val,
            ttl_seconds = EXPIRE_URL_DATA)            
        cache.set(OPM_JSON_URL + ".cachecount",
            "1",
            ttl_seconds = EXPIRE_URL_DATA)
    ...
    applies_to_text = render.Text(content=opm_applies_to_date,
        color=DARK_BLUE)
    
    # build widget from all rows of above text
    all_rows = render.Column(children = [status_title_text, status_text,
                                        for_date_text, applies_to_text])
