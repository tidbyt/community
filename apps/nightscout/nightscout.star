...
Description: Displays Continous Glucose Monitoring (CGM) data from the Nightscout Open Source project (https://nightscout.github.io/).
...
def main(config):
    ...
    if nightscout_id == None:
    ...
    elif re.match('[^a-zA-Z0-9]', nightscout_id): 
        ...
        return render.Root(
            render.Marquee(
                width = 64,
                child = render.Text(
                    content=str("Nightscout ID contains invalid characters")
                )
            )
        )
    ...
    if nightscout_data_cached != None:
        ...
        if resp.status_code == 502:
            ...
            fail("Failed to retieve the Nightscout details with status " + str(resp.status_code))
                    ...
                    render.Column(
                        ...
                        children = [
                            ...
                            render.Marquee(
                                width = 32,
                                child=render.Text(
                                    # This time is currently in UTC - need to figure out how to display in a selected timezone.
                                    content=latest_reading_dt.format("2006-01-02 03:04:05 PM"),
                                    font="tom-thumb"
                                )
                            )
                        ]
