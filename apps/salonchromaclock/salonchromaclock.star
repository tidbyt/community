...
def main(config):
    ...
    DEFAULT_IS_US_DATE_FORMAT = True

    now = time.now().in_location(timezone)

    is_us_date_format = config.bool("is_us_date_format", DEFAULT_IS_US_DATE_FORMAT)
    now_date = now.format("2 January 2006").upper()
    if is_us_date_format:
        now_date = now.format("January 2 2006").upper()
            ...
            render.Row(
                ...
                children = [
                    ...
                    render.Column(
                        ...
                        main_align = "space_evenly",
                        children = [
                            render.Marquee(width = 42, scroll_direction = "horizontal", child = render.Text(content = day + ", " + now_date, color = "#c01e30", font = "CG-pixel-4x5-mono")),
                            render.Marquee(width = 42, scroll_direction = "horizontal", child = render.Text(now.format(time_format_separator), color = "#ffffff", font = "Dina_r400-6")),
                        ],
                    ),
                ],
            ),
