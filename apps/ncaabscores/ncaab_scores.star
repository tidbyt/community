"""
Applet: NCAAB Scores
Summary: Displays NCAA Baseball scores
Description: Displays live and upcoming NCAA Baseball scores from a data feed.
Author: LunchBox8484
"""
    ...
    if len(scores) > 0:
        ...
        for i, s in enumerate(scores):
            ...
            if gameStatus == "post":
                ...
                checkNotesLength = 0
                if checkNotes != "NO":
                    checkNotesLength = len(competition["notes"])
                if checkSeries != "NO":
                    seriesSummary = competition["series"]["summary"]
                    gameTime = seriesSummary.replace("series ", "")
                if checkNotesLength > 0 and checkSeries == "NO":
