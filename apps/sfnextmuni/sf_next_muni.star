...
def main(config):
    ...
    if type(routes) != 'list': routes = [routes]

    prediction_map = {}

    for route in routes:
        if 'routeTag' not in route or 'direction' not in route: continue
        routeTag = route['routeTag']

        destinations = route['direction']
        if type(destinations) != 'list': destinations = [destinations]
        for dest in destinations:
            if 'title' not in dest or 'prediction' not in dest: continue
            ...
            if routeTag == 'KT': routeTag = 'T' if 'Inbound' in dest['title'] else 'K'
