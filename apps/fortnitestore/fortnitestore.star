  ...
  if api_key:
    ...
    return render.Root(
      render.Stack(
        children=[
          render.Column(
            main_align="end", children=[render.Image(src=image_resp.body(), height=32)]),
          render.Marquee(
            width=64, 
            offset_start=32,
            offset_end=32,
            align="end",
            child=render.Padding(pad=(0, 2, 2, 0),
              child=render.Text(
                content=items[picked]["name"], color=color_key[items[picked]["rarity"].lower()]
              )
            ),
          ),
          render.Column(
