...
def get_random_number(quote_num):
    # because this language lacks random integer generation :(
    url = "https://www.random.org/integers/?num=1&min=1&max=%d&rnd=new&format=plain&col=1&base=10" % quote_num
    resp = http.get(url)
    if resp.status_code != 200:
        fail("Request failed with status %d" % resp.status_code)
    return int(resp.body().strip())
...
def main():
    rand_num = get_random_number(len(quotes))
