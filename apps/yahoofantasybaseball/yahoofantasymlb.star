...
TIDBYT_OAUTH_CALLBACK_URL = "https%3A%2F%2Fappauth.tidbyt.com%2Fyahoofantasymlb"

YAHOO_CLIENT_ID = "dj0yJmk9RkJhYkJhWmFsaWREJmQ9WVdrOVNXRkhOVVI2YkVrbWNHbzlNQT09JnM9Y29uc3VtZXJzZWNyZXQmc3Y9MCZ4PWRl"
YAHOO_CLIENT_SECRET = secret.decrypt("AV6+xWcEpOfd72G+TFyN/8p3P0jWeqUaG5JQaL8R0+aiipivAMaUNWyRS8NWrbO3JVRHPzNYygrUWkUfFfU4xyD6KhfeOYZY/uKvq3MQFBjrs/tGP7wvIzxMmyrdLZtfEa5qclyWEr+ciYWbbN+78GaE8cVlmCe42P2DX9saJatyn0n+us51SoDkq/P/Ag==") or ""
...
def main(config):
    ...
    colorScheme = config.get("colorScheme", '["BD3039", "0C2340", "BD3039", "0C2340", "FFFFFF"]')
                        ...
                        renderCategory.extend(
                            [
                                render.Column(
                                    ...
                                    children = [
                                        render.Column(
                                            children = render_standings(x, standings, entriesToDisplay, topColor, colorScheme, league_name),
                                        ),
                                    ],
                                ),
                            ],
...
def render_standings(x, standings, entriesToDisplay, topColor, colorScheme, leagueName):
    output = []
    teamTies = ""
    teamWins = ""
    teamLosses = ""
    ...
    for i in range(entriesToDisplay):
        if i + x < len(standings):
            ...
            team = render.Column(
                children = [
                    render.Box(width = 64, height = containerHeight, color = teamColor, child = render.Row(expanded = True, main_align = "start", cross_align = "start", children = [
