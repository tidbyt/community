...
def get_remote_assignments(api_token, course_id):
    api_url = "https://canvas.instructure.com/api/v1/courses/" + str(
        course_id) + "/assignments?access_token=" + api_token
    rep = http.get(api_url)
    if rep.status_code != 200:
        return [], "Can not Connect to Canvas"
    data = rep.json()
    return [(assignment["due_at"], assignment["name"] + "")
            for assignment in data if assignment["due_at"] != None
            and (time.parse_time(assignment["due_at"]) - time.now()).hours > 0
            and (time.parse_time(assignment["due_at"]) - time.now()).hours < 200]
