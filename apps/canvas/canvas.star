        ...
        children = [
            ...
            render.Column(
                children = [
                    render.Marquee(
                        width = 40,
                        child = render.Text(assignment_name),
                    ),
                    render.Text(due_date[5:10], color = "#d52e3a", font = "5x8"),
                ],
            ),
        ],
    )
...
def getcourse(domain, api_token):
    ...
    if class_cache != None:
        ...
        for course in data:
            ...
            # Check Dates of Course
            if "created_at" in course:
                time_stamp = course["created_at"]
...
def main(config):
    ...
    if len(canvas_data) == 1:
        return render.Root(
            child = render.Column(
                children = [
                    showEvent(canvas_data[0], timezone),
        ...
        return render.Root(
            child = render.Column(
                children = [
                    showEvent(canvas_data[index], timezone),
                    render.Box(width = 100, height = 1, color = "#ffffff"),
                    showEvent(canvas_data[index + 1], timezone),
                ],
            ),
        )
    ...
    return schema.Schema(
        ...
        fields = [
            schema.Text(
                id = "api_token",
                name = "Canvas API Key",
                desc = "API Key Generated from Canvas",
                icon = "key",
                default = "",
            ),
            schema.Text(
                id = "domain",
                name = "Canvas Domain",
                desc = "e.g. canvas.instructure.com or canvas.uw.edu",
                icon = "sitemap",
