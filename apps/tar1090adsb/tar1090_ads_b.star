...
def lookup_aircraft_desc(tar_url, aircraft_data, db_version):
    ...
    if aircraft_types != None:
        aircraft_types = json.decode(aircraft_types)
    else:
        ...
        cache.set("db-%s/%s" % (db_version, "icao_aircraft_types"), response.body(), ttl_seconds = 86400)
...
def lookup_db(tar_url, icao, level, db_version):
    ...
    aircraft_db = cache.get("db-%s/%s" % (db_version, bkey))

    if aircraft_db != None:
        aircraft_db = json.decode(aircraft_db)
    else:
        print("Missed aircraft_db cache for DB file " + bkey)
...
def get_aircraft_icon(category, designator, description, addrtype, color):
    ...
    if aircraft_icon != None:
        return base64.decode(aircraft_icon)
    else:
...
def main(config):
    ...
    if aircraft_data == None and retried_database_version == False:
        ...
        db_version = get_db_version(True)
        aircraft_data = lookup_db(tar_url, aircraft["hex"], 1, db_version)
    if aircraft_data == None and retried_database_version == True:
        return unable_to_reach_tar_error(tar_url)
...
# From https://github.com/posburn/tidbyt-apps/blob/main/CustomMarquee/custommarquee.star
...
def wrapped_text(items, width, color, offset, font, has_shadow, center):
    children = []
    if has_shadow:
        children.append(
            render.Padding(
...
def custom_marquee(text = "", width = 64, height = 32, rate = 100, color = "#fff", font = "tb-8", shadow = False, center = False, delay = 0, default_duration = 6):
    ...
    if lines < max_lines:
        ...
        if center == True:
            return frames(
                wrapped_frames,
                render.Box(
                    child = wrapped_text(items, width, color, 0, font, shadow, center),
                ),
    ...
    for i in range(duration(seconds, rate)):
